...
Legacy yellow-pipeline helpers from the chemistry pipeline. Keep the structure
for now (schema validation, gating helpers, Parquet emission) but plan to adapt
it for cybersecurity sources with secret/PII/exploit scrubbing.
...
def main() -> None:
    ...
    ap.add_argument("--pools-root", default="/data/cyber/pools")
    ...
    logs_dir = Path("/data/cyber/_logs").expanduser()
    ensure_dir(logs_dir)
