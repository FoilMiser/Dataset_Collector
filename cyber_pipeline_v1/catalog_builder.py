...
Usage:
  python catalog_builder.py --targets targets.yaml --output /data/cyber/_catalogs/global_catalog.json
...
tiktoken_mod = importlib.import_module("tiktoken") if tiktoken_spec else None
...
def estimate_tokens(text: str, model: str = "cl100k_base") -> int:
    ...
    if tiktoken_mod:
        enc = tiktoken_mod.get_encoding(model)
...
def main() -> None:
    ...
    permissive_root = Path(pools_cfg.get("permissive", "/data/cyber/pools/permissive")).expanduser()
    copyleft_root = Path(pools_cfg.get("copyleft", "/data/cyber/pools/copyleft")).expanduser()
    quarantine_root = Path(pools_cfg.get("quarantine", "/data/cyber/pools/quarantine")).expanduser()
    manifests_root = Path(globals_cfg.get("manifests_root", "/data/cyber/_manifests")).expanduser()
