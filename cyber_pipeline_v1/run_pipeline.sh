...
#
# Wrapper script for the Cybersecurity + ICS Corpus Pipeline.
...
usage() {
    cat << EOF
Cybersecurity Corpus Pipeline v${VERSION}
...
Options:
  ...
  --stage STAGE           Run specific stage: all, classify, download, review, catalog
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Cybersecurity Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
print(cfg.get("globals", {}).get("queues_root", "/data/cyber/_queues"))
PY
)
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/cyber/_queues'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/cyber/_catalogs'))")
