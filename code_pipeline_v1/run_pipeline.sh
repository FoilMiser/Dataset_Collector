...
#
# Usage:
#   ./run_pipeline.sh --targets targets_code.yaml                    # Dry-run
#   ./run_pipeline.sh --targets targets_code.yaml --execute          # Full run
#   ./run_pipeline.sh --targets targets_code.yaml --stage download   # Specific stage
#
# Stages:
...
Required:
  --targets FILE          Path to targets_code.yaml
...
Examples:
  ...
  $0 --targets targets_code.yaml

  # Execute with limits
  $0 --targets targets_code.yaml --execute --limit-targets 3 --limit-files 5

  # Run only classification stage
  $0 --targets targets_code.yaml --stage classify

  # Emit a YELLOW review plan
  $0 --targets targets_code.yaml --stage yellow
...
run_download() {
    ...
    # Get queues root from targets_code.yaml
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/logic/_queues'))")
