...
KG-nav extensions:
  - `s3_sync` for OpenAlex snapshots
  - `aws_requester_pays` for Crossref public data file
  - `torrent` for Academic Torrents / magnet links
  - `figshare` helper for OpenCitations/ORCID dumps

Supported strategies: http, ftp, git, zenodo, dataverse, huggingface_datasets,
figshare, s3_sync, aws_requester_pays, torrent
...
def resolve_pool_dir(ctx: DownloaderContext, pool_name: str, target_id: str, dest_subdir: Optional[str]) -> Path:
...
def http_download_with_resume(ctx: DownloaderContext, url: str, out_path: Path,
    ...
    for attempt in range(ctx.retry.max_attempts):
        ...
        try:
            headers = {"User-Agent": f"kg-nav-downloader/{VERSION}"}
            if start_byte > 0:
...
def handle_zenodo(ctx: DownloaderContext, target: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    ...
    for attempt in range(ctx.retry.max_attempts):
        try:
            r = requests.get(api_url, timeout=60, headers={"User-Agent": f"kg-nav/{VERSION}"})
            r.raise_for_status()
            meta = r.json()
            break
        except Exception as e:
...
def handle_figshare(ctx: DownloaderContext, target: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    if requests is None:
        return [{"status": "error", "error": "requests not installed"}]

    urls = target.get("download", {}).get("urls") or target.get("download", {}).get("articles") or []
    if not urls:
        return [{"status": "noop", "reason": "no figshare urls"}]

    ensure_dir(out_dir)
    results: List[Dict[str, Any]] = []

    for url in urls:
        article_id = _extract_figshare_id(url)
        if not article_id:
            results.append({"status": "noop", "reason": "no article id", "url": url})
            continue

        if not ctx.mode.execute:
            results.append({"status": "planned", "article_id": article_id})
            continue

        try:
            api_url = f"https://api.figshare.com/v2/articles/{article_id}/files"
            resp = requests.get(api_url, timeout=60, headers={"User-Agent": f"kg-nav-figshare/{VERSION}"})
            resp.raise_for_status()
            files = resp.json()
        except Exception as e:
            results.append({"status": "error", "article_id": article_id, "error": repr(e)})
            continue

        max_files = ctx.limits.limit_files or len(files)
        for idx, fmeta in enumerate(files[:max_files]):
            dl_url = fmeta.get("download_url") or fmeta.get("suppl_url") or ""
            fname = fmeta.get("name") or f"figshare_file_{idx}"
            size = fmeta.get("size") or None
            if not dl_url:
                results.append({"status": "error", "article_id": article_id, "error": "missing download_url"})
                continue
            out_path = out_dir / safe_name(fname)
            info = http_download_with_resume(ctx, dl_url, out_path, expected_size=size)
...
def handle_s3_sync(ctx: DownloaderContext, target: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    urls = target.get("download", {}).get("urls") or []
    if not urls:
        return [{"status": "noop", "reason": "no s3 urls"}]

    ensure_dir(out_dir)
    results: List[Dict[str, Any]] = []
    no_sign = bool(target.get("download", {}).get("no_sign_request", True))
    extra = target.get("download", {}).get("extra_args") or []

    for url in urls:
        if not ctx.mode.execute:
            results.append({"status": "planned", "url": url, "dest": str(out_dir)})
            continue
        ...
        try:
            run_cmd(cmd)
            results.append({"status": "ok", "url": url, "dest": str(out_dir)})
        except Exception as e:
...
def handle_aws_requester_pays(ctx: DownloaderContext, target: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    dl = target.get("download", {})
    bucket = dl.get("bucket")
    key = dl.get("key")
    urls = dl.get("urls") or []
    extra = dl.get("extra_args") or []

    if not bucket and urls:
        parsed = urls[0].replace("s3://", "").split("/", 1)
        if len(parsed) == 2:
            bucket, key = parsed

    if not bucket or not key:
        return [{"status": "noop", "reason": "bucket/key required for requester_pays"}]

    ensure_dir(out_dir)
    dest = out_dir / Path(key).name

    if not ctx.mode.execute:
        return [{"status": "planned", "bucket": bucket, "key": key, "dest": str(dest)}]
    ...
    try:
        run_cmd(cmd)
        result = {"status": "ok", "bucket": bucket, "key": key, "path": str(dest)}
        if ctx.mode.verify_sha256:
            result["sha256"] = sha256_file(dest)
        return [result]
    except Exception as e:
        return [{"status": "error", "error": repr(e), "bucket": bucket, "key": key}]


def handle_torrent(ctx: DownloaderContext, target: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    urls = target.get("download", {}).get("urls") or []
    if not urls:
        return [{"status": "noop", "reason": "no torrent/magnet urls"}]

    ensure_dir(out_dir)
    results: List[Dict[str, Any]] = []
    for url in urls:
        if not ctx.mode.execute:
            results.append({"status": "planned", "url": url})
            continue
        cmd = ["aria2c", "--seed-time=0", "--dir", str(out_dir), url]
        try:
            run_cmd(cmd)
            results.append({"status": "ok", "url": url, "dest": str(out_dir)})
        except Exception as e:
...
def run_target(ctx: DownloaderContext, row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dest_subdir = (download.get("dest_subdir") or "").strip()
    out_dir = resolve_pool_dir(ctx, pool, tid, dest_subdir if dest_subdir else None)
...
def main() -> None:
    ...
    ap.add_argument("--pools-root", default="/data/kg_nav/pools", help="Fallback pools root")
    ap.add_argument("--logs-dir", default="/data/kg_nav/_logs", help="Logs directory")
    ...
    mode = RunMode(
        ...
        verify_zenodo_md5=args.verify_zenodo_md5,
        enable_resume=not args.no_resume,
        workers=args.workers,
    )

    pools_root = Path(args.pools_root).expanduser()
    pools = pools_from_targets_yaml(Path(args.targets_yaml) if args.targets_yaml else None, pools_root)
    logs_dir = Path(args.logs_dir).expanduser()
    ensure_dir(logs_dir)

    ctx = DownloaderContext(pools=pools, logs_dir=logs_dir, limits=limits, mode=mode, retry=retry)

    rows = read_jsonl(Path(args.queue))
    if limits.limit_targets:
        rows = rows[: limits.limit_targets]

    if mode.workers <= 1:
        for r in rows:
            run_target(ctx, r)
    else:
        with ThreadPoolExecutor(max_workers=mode.workers) as ex:
            futures = {ex.submit(run_target, ctx, r): r["id"] for r in rows}
