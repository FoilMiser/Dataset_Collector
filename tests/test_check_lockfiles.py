...
from tools import check_lockfiles
...
def test_lockfile_check_passes(tmp_path: Path, monkeypatch) -> None:
...
def test_lockfile_check_reports_missing_pin(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.chdir(tmp_path)
...
def test_lockfile_check_reports_missing_include(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.chdir(tmp_path)
    _write(tmp_path / "requirements.in", "requests>=2.0\n")
    _write(tmp_path / "requirements-dev.in", "-r requirements.in\npytest-cov\n")
