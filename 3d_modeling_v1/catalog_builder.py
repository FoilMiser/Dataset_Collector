...
Usage:
  python catalog_builder.py --targets targets.yaml --output /data/3d/_catalogs/global_catalog.json
...
def main() -> None:
    ...
    permissive_root = Path(pools_cfg.get("permissive", "/data/3d/pools/permissive")).expanduser()
    copyleft_root = Path(pools_cfg.get("copyleft", "/data/3d/pools/copyleft")).expanduser()
    quarantine_root = Path(pools_cfg.get("quarantine", "/data/3d/pools/quarantine")).expanduser()
    manifests_root = Path(globals_cfg.get("manifests_root", "/data/3d/_manifests")).expanduser()
