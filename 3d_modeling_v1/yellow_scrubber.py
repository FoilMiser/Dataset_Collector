...
3D adaptation notes:
- This module will be extended to handle mesh validation/sanitization and record-level
  license filtering for community 3D datasets (Thingi10K/Objaverse/etc.).
- Legacy PubChem/PMC handlers remain for compatibility with the upstream prototype;
  swap these with mesh workers as you implement the 3D-specific gates.
...
# --------------------------
# 3D record-level filtering helpers
# --------------------------

def load_denylist(path: Path) -> Dict[str, Any]:
    if not path.exists():
        return {"patterns": []}
    return read_yaml(path) or {"patterns": []}

def pattern_blocked(patterns: List[Dict[str, Any]], text: str) -> bool:
    import re
    for p in patterns:
        ptype = (p.get("type") or "substring").lower()
        val = p.get("value") or ""
        if not val:
            continue
        if ptype == "regex":
            try:
                if re.search(val, text, flags=re.IGNORECASE):
                    return True
            except re.error:
                continue
        else:
...
def record_level_filter(
    metadata_files: List[Path],
    output_dir: Path,
    allow_spdx: List[str],
    deny_phrases: List[str],
    license_map: Dict[str, Any],
    deny_patterns: List[Dict[str, Any]],
) -> Dict[str, Any]:
    ...
    for meta_path in metadata_files:
        ...
        for row in rows:
            lic_blob = row.get("license", "") or row.get("license_spdx", "") or ""
            spdx = normalize_spdx_from_text(license_map, json.dumps(row), lic_blob)
            hit_phrases = restriction_phrase_hits(license_map, json.dumps(row))
            deny_hit = any(ph in hit_phrases for ph in deny_phrases)
            name_blob = f"{row.get('name','')} {row.get('tags','')}"
            pattern_hit = pattern_blocked(deny_patterns, name_blob)

            if spdx not in allow_spdx or deny_hit or pattern_hit:
                skipped.append({"record_id": row.get("id"), "spdx": spdx, "reason": "policy_block"})
                continue

            # Copy asset if available
            src_path = row.get("path") or row.get("file_path")
            if src_path:
                src = Path(src_path)
                if src.exists():
                    rel = src.name
                    ...
                    row["path"] = str(dest)
            row["license_spdx"] = spdx
            filtered.append(row)

    if filtered:
        out_jsonl = output_dir / "filtered_records.jsonl"
...
def main() -> None:
    ...
    ap.add_argument("--pools-root", default="/data/3d/pools")
    ...
    # 3D mesh / record-level filter controls
    ap.add_argument("--record-filter-target", default=None, help="Target id to filter at record level (e.g., thingi10k_raw)")
    ap.add_argument("--record-filter-input", default=None, help="Input dir containing metadata + assets for record-level filtering")
    ap.add_argument("--record-filter-output", default=None, help="Output dir for filtered assets/metadata")
    ap.add_argument("--weapon-filter", action="store_true", help="Apply weapon/trademark filters from denylist.yaml")
    args = ap.parse_args()
    ...
    denylist_path = Path(companion.get("denylist", "./denylist.yaml")).expanduser().resolve()
    ...
    if args.record_filter_target:
        target_def = target_defs.get(args.record_filter_target, {})
        policy = target_def.get("record_level_policy", {})
        allow_spdx = policy.get("allow_spdx") or license_map.get("spdx", {}).get("allow", [])
        deny_phrases = policy.get("deny_if_contains_phrases", [])
        deny_patterns = denylist.get("patterns", [])

        input_dir = Path(args.record_filter_input or pools.quarantine / args.record_filter_target).expanduser()
        output_dir = Path(args.record_filter_output or pools.permissive / f"{args.record_filter_target}_filtered").expanduser()
        metadata_files = list(input_dir.glob("*.jsonl"))
        ensure_dir(output_dir)

        if not metadata_files:
            run_report["outputs"].append(
                {"record_filter": "skipped", "reason": f"no metadata jsonl in {input_dir}"}
            )
        else:
            rf_result = record_level_filter(
                metadata_files=metadata_files,
                output_dir=output_dir,
                allow_spdx=allow_spdx,
                deny_phrases=deny_phrases,
                license_map=license_map,
                deny_patterns=deny_patterns if args.weapon_filter else [],
            )
            run_report["record_filter_ran"] = True
            run_report["outputs"].append({"record_filter": rf_result})

    logs_dir = Path("/data/3d/_logs").expanduser()
    ensure_dir(logs_dir)
    ...
    if args.record_filter_target:
        print(f"Record filter ({args.record_filter_target}): {'ran' if run_report['record_filter_ran'] else 'skipped'}")
