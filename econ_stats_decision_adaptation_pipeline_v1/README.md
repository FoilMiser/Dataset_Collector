# Economic Statistics & Decision Science Pipeline (v1.0)

A safety-first **prototype** pipeline for building an economics/statistics/decision-science training corpus from open datasets, open textbooks, and regulation/compliance-aligned releases, with strong emphasis on **license compliance, provenance tracking, and safe-by-default execution**.

Adapted from `chem_pipeline_v1`, this package adds domain-specific safeguards for measurement-first sources:
- tabular-first targets (time series, panels, SDMX/CKAN/Socrata placeholders) with planned `tabular_normalize` and `zip_unpack` build stages,
- explicit **advocacy/partisanship** filters to avoid propagandistic corpora,
- stronger defaults for **PII/microdata** avoidance and license evidence capture.

This repo helps you:
- keep a single inventory (`targets.yaml` / `targets_econ_stats_decision.yaml`) of candidate sources,
...
- and build a global catalog / training manifests that respect output pools (`permissive`, `copyleft`, `quarantine`).
...
### 1) Economics/statistics/decision-science adaptation
- New `targets_econ_stats_decision.yaml` focusing on public-domain and CC-licensed measurement-first sources (BLS, BEA, EIA, World Bank, open textbooks) with explicit **output_pool** routing.
- Defaults tuned for `/data/econ` pools, queues, and manifests.
- Early hooks for `tabular_normalize`, SDMX/CKAN/Socrata resolvers, `advocacy_source_filter`, and `pii_scan` gates called out in target definitions.

### 2) License confidence + change detection (carried over)
- SPDX normalization returns a confidence score and will **force YELLOW** when below a configurable threshold.
- Evidence snapshots track SHA256 digests and automatically flag **license content changes** for manual review.
- `pipeline_driver.py` now propagates `output_pool` into queue rows so downloads land in the intended pool.

### 3) Documentation + checklists
- Adaptation plan documented in `ECON_STATS_DECISION_ADAPTATION.md` to guide future SDMX/tabular work.
- Version metadata retained at v1.0 across manifests and wrapper script.
...
# or:
python3 review_queue.py --queue /data/econ/_queues/yellow_pipeline.jsonl list
```
...
### Build catalog
```bash
python3 catalog_builder.py --targets targets.yaml --output /data/econ/_catalogs/global_catalog.json
```
