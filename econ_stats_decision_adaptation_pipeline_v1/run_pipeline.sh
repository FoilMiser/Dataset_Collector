...
#
# Single wrapper script for the Economics / Statistics / Decision Science corpus pipeline.
...
usage() {
    cat << EOF
Economic Statistics & Decision Science Pipeline v${VERSION}
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Economic Statistics & Decision Science Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/econ/_queues'))")
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/econ/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/econ/_catalogs'))")
