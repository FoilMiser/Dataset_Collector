...
def resolve_source_url(row: dict[str, Any]) -> Optional[str]:
    download = normalize_download(row.get("download", {}) or {})
    url = download.get("url")
    if url:
        return url
...
def extract_artifacts(ctx: AcquireContext, row: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    ...
    for idx, path in enumerate(files):
        ...
        if not text:
            ...
            continue
        chunks = chunk_text(text, ctx.chunking.max_chars, ctx.chunking.min_chars)
        if not chunks:
...
def run_target(ctx: AcquireContext, bucket: str, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not handler or strat in {"none", ""}:
        ...
        try:
            manifest["results"] = handler(ctx, row, artifacts_dir)
        except Exception as e:
            manifest["results"] = [{"status": "error", "error": repr(e)}]

    if ctx.mode.execute:
        manifest["extraction"] = extract_artifacts(ctx, row, out_dir)
...
def load_config(targets_path: Optional[Path], overrides: argparse.Namespace) -> tuple[Roots, ChunkingConfig]:
    cfg: dict[str, Any] = {}
    ...
    logs_root = Path(overrides.logs_root or g.get("logs_root", "/data/metrology/_logs"))
    text_defaults = g.get("text_processing_defaults", {}) or {}
    chunking = ChunkingConfig(
        max_chars=int(text_defaults.get("max_chunk_chars", 7000)),
        min_chars=int(text_defaults.get("min_chunk_chars", 600)),
    )
    roots = Roots(raw_root=raw_root.expanduser().resolve(), manifests_root=manifests_root.expanduser().resolve(), logs_root=logs_root.expanduser().resolve())
    return roots, chunking
...
def main() -> None:
    ...
    ap.add_argument("--targets-yaml", default=None, help="Path to targets_metrology.yaml for roots")
    ...
    roots, chunking = load_config(targets_path, args)
