...
A safety-first pipeline for building an ethical metrology and standards training corpus. Version 2 restructures the metrology pipeline to match the **docs/PIPELINE_V2_REWORK_PLAN.md** flow:
...
| Acquire | `dc run --stage acquire` | Downloads payloads into `raw/{green|yellow}/{license_pool}/{target_id}` and extracts PDF/HTML into JSONL chunks. Dry-run by default; `--execute` performs downloads. |
...
dc pipeline metrology -- --targets ../pipelines/targets/targets_metrology.yaml --stage classify --no-fetch

# Acquire GREEN and YELLOW (execute downloads)
dc run --pipeline metrology --stage acquire --allow-data-root -- --queue /data/metrology/_queues/green_pipeline.jsonl --bucket green --targets-yaml ../pipelines/targets/targets_metrology.yaml --execute
dc run --pipeline metrology --stage acquire --allow-data-root -- --queue /data/metrology/_queues/yellow_pipeline.jsonl --bucket yellow --targets-yaml ../pipelines/targets/targets_metrology.yaml --execute

# Screen, merge, catalog
dc run --pipeline metrology --stage yellow_screen --allow-data-root -- --queue /data/metrology/_queues/yellow_pipeline.jsonl --targets ../pipelines/targets/targets_metrology.yaml --execute
dc run --pipeline metrology --stage merge --allow-data-root -- --targets ../pipelines/targets/targets_metrology.yaml --execute
dc catalog-builder --pipeline metrology --allow-data-root -- --targets ../pipelines/targets/targets_metrology.yaml --output /data/metrology/_catalogs/catalog.json
```
