...
Reads:
  - targets_metrology.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    metrology_routing = (target.get("metrology_routing", {}) or {})
    legacy_routing = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or metrology_routing.get("subject") or legacy_routing.get("subject") or "metrology"
    domain = routing.get("domain") or metrology_routing.get("domain") or legacy_routing.get("domain")
    category = routing.get("category") or metrology_routing.get("category") or legacy_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(metrology_routing.get("level"), coerce_int(legacy_routing.get("level"))))
    granularity = routing.get("granularity") or metrology_routing.get("granularity") or legacy_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"metrology-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_metrology.yaml (v0.8)")
    ...
    for t in targets:
        ...
        mr = t.get("math_routing", {}) or t.get("metrology_routing", {}) or t.get("routing", {}) or {}
    ...
    write_jsonl(queues_root / "green.jsonl", green_rows)
    write_jsonl(queues_root / "yellow.jsonl", yellow_rows)
