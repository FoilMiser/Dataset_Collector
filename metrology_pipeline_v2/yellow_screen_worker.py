...
def resolve_roots(cfg: dict[str, Any]) -> Roots:
    ...
    return Roots(
        ...
        manifests_root=Path(g.get("manifests_root", "/data/metrology/_manifests")),
        ledger_root=Path(g.get("ledger_root", "/data/metrology/_ledger")),
        pitches_root=Path(g.get("pitches_root", "/data/metrology/_pitches")),
    )
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_metrology.yaml")
