...
Cybersecurity-focused staging of the math pipeline v2 layout. This version keeps the safety-first flow (evidence → queues → acquisition → screening → merge → difficulty → catalog) while adopting cyber defaults, roots, and helper workers.
...
5) **Difficulty** assignment + final light screen (`difficulty_worker.py`).
6) **Catalog** counts and shard summaries (`catalog_builder.py`).
...
`run_pipeline.sh` orchestrates these stages with sensible defaults (`--stage classify|review|acquire_green|acquire_yellow|screen_yellow|merge|difficulty|catalog`).
