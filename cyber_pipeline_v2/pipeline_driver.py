...
Reads:
  - targets_cyber.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    legacy_routing = (target.get("cyber_routing") or target.get("math_routing") or {}) or {}

    subject = routing.get("subject") or legacy_routing.get("subject") or "cyber"
    domain = routing.get("domain") or legacy_routing.get("domain")
    category = routing.get("category") or legacy_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(legacy_routing.get("level")))
    granularity = routing.get("granularity") or legacy_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"cyber-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_cyber.yaml (v0.8)")
