"""
Math pipeline helper for YELLOW bucket planning and manual review prep.

This script prepares manual review plans for YELLOW targets. Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_cyber.yaml
    python yellow_scrubber.py --targets targets_cyber.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Cyber pipeline YELLOW review helper")
    ap.add_argument("--targets", required=True, help="Path to targets_cyber.yaml")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/cyber/_queues")).expanduser()
    ...
    print("Cyber YELLOW queue summary")
