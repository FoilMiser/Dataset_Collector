#!/usr/bin/env bash
#
# run_pipeline.sh (code v2.0)
#
# Wrapper for the Code Corpus Pipeline v2 stages.
#
set -euo pipefail
...
usage() {
  cat << EOF
  Code Corpus Pipeline v${VERSION}
...
print(cfg.get("globals", {}).get("queues_root", "/data/code/_queues"))
PY
)
...
import yaml
cfg = yaml.safe_load(open("${TARGETS}"))
print(cfg.get("globals", {}).get("catalogs_root", "/data/code/_catalogs"))
PY
)
