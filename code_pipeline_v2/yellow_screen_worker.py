...
class Sharder:
    ...
    def flush(self) -> Optional[Path]:
        ...
        append_jsonl(path, self.current_rows)
        self.current_rows = []
        return path
...
def resolve_roots(cfg: dict[str, Any]) -> Roots:
    ...
    return Roots(
        ...
        manifests_root=Path(g.get("manifests_root", "/data/code/_manifests")),
        ledger_root=Path(g.get("ledger_root", "/data/code/_ledger")),
        pitches_root=Path(g.get("pitches_root", "/data/code/_pitches")),
    )
...
def merge_screening_config(cfg: dict[str, Any], target: dict[str, Any]) -> ScreeningConfig:
    ...
    return ScreeningConfig(
        text_fields=list(t_screen.get("text_field_candidates") or g_screen.get("text_field_candidates") or ["text", "code", "content"]),
    ...
    for k in candidates:
        if k in row and row[k]:
            ...
            if isinstance(val, (list, tuple)):
                val = "\n".join(map(str, val))
            return str(val)
        if "source" in row and isinstance(row["source"], dict) and row["source"].get(k):
            val = row["source"][k]
            if isinstance(val, (list, tuple)):
                val = "\n".join(map(str, val))
            return str(val)
...
def resolve_routing(raw: dict[str, Any], queue_row: dict[str, Any]) -> dict[str, Any]:
    rt = (raw.get("routing") or raw.get("math_routing") or {}) or {}
    routing = {
        "subject": rt.get("subject") or queue_row.get("routing_subject") or "code",
        "domain": rt.get("domain") or queue_row.get("routing_domain") or "multi",
...
def canonical_record(raw: dict[str, Any], text: str, target_id: str, license_profile: str, license_spdx: Optional[str], routing: dict[str, Any]) -> dict[str, Any]:
    ...
    for pool in pools:
        ...
        for file_path in iter_raw_files(raw_dir):
            for raw in read_jsonl(file_path):
                ...
                routing = resolve_routing(raw, queue_row)
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_code.yaml")
    ...
    queue_rows = list(read_jsonl(Path(args.queue)))
