...
def level_from_routing(diff_cfg: Dict[str, Any], routing: Dict[str, Any]) -> Optional[int]:
    subj = (routing or {}).get("subject") or "code"
...
def select_payload(record: Dict[str, Any]) -> str:
...
def heuristic_level(text: str) -> int:
    ...
    loc = len(lines)
    avg_len = sum(len(l) for l in lines) / max(1, loc)
    import_count = sum(1 for l in lines if re.match(r"\s*(import|from\s+\w+)", l))
    brace_depth = max((l.count("{") + l.count("(") + l.count("[")) for l in lines) if lines else 0
...
def assign_difficulty(diff_cfg: Dict[str, Any], record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    level = heuristic_level(select_payload(record))
