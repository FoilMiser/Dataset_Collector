...
Outputs:
  ...
  - _ledger/difficulty_index.jsonl
  - _pitches/difficulty_pitched.jsonl (optional pitched samples)
"""
...
def resolve_roots(cfg: Dict[str, Any]) -> Roots:
    ...
    return Roots(
        combined_root=Path(g.get("combined_root", "/data/safety/combined")),
        final_root=Path(g.get("final_root", "/data/safety/final")),
...
def level_from_routing(diff_cfg: Dict[str, Any], routing: Dict[str, Any]) -> Optional[int]:
    subj = (routing or {}).get("subject") or "safety_incident"
...
def heuristic_level_safety(text: str) -> int:
    base = heuristic_level(text)
    lower = text.lower()
    boost = 0
    boost_two = ["iec 61508", "monte carlo", "bayesian", "pra", "probabilistic risk", "formal verification"]
    boost_one = ["stpa", "stamp", "fault tree", "event tree", "sil", "hazop", "bow-tie", "bow tie"]
    if any(k in lower for k in boost_two):
        boost = 2
    elif any(k in lower for k in boost_one):
        boost = 1
    if any(k in lower for k in ["bulletin", "recall", "alert", "advisory"]) and len(text) < 1500:
        base = min(base, 4)
...
def assign_difficulty(diff_cfg: Dict[str, Any], record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    level = heuristic_level_safety(record.get("text", ""))
    return {"level": level, "method": "heuristic", "confidence": 0.5}
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="targets_safety_incident.yaml")
    ...
    for rec in iter_combined_records(roots):
        ...
        if len(text) < screen_cfg.min_chars or len(text) > screen_cfg.max_chars:
            summary["pitched"] += 1
            if args.execute:
                append_jsonl(roots.pitches_root / "difficulty_pitched.jsonl", [{"record_id": rec.get("record_id"), "reason": "length_bounds"}])
            continue
        ...
        if args.execute:
            ...
            append_jsonl(roots.ledger_root / "difficulty_index.jsonl", [{
