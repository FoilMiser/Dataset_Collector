...
Reads:
  - targets_safety_incident.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
v2.0 changes:
  - Adds generic routing fields for difficulty assignment
  - Aligns output queues with v2 layout
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    safety_routing = (target.get("safety_routing", {}) or {})

    subject = routing.get("subject") or safety_routing.get("subject") or "safety_incident"
    domain = routing.get("domain") or safety_routing.get("domain")
    category = routing.get("category") or safety_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(safety_routing.get("level")))
    granularity = routing.get("granularity") or safety_routing.get("granularity") or "target"
    ...
    return {
        ...
        "confidence": routing.get("confidence") or safety_routing.get("confidence"),
        "reason": routing.get("reason") or safety_routing.get("reason"),
    }
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"safety-incident-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_safety_incident.yaml (v0.8)")
    ...
    for t in targets:
        ...
        evaluation = {
            ...
            "safety_routing": t.get("safety_routing", {}),
            "routing": routing,
        }
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "difficulty_level": routing.get("level"),
            # Generic routing (v2)
