#!/usr/bin/env python3
"""
pmc_worker.py (v2.0 helper)

Downloads and chunks allowlisted PMC Open Access articles into the v2 raw layout.

v2.0 notes:
  - Outputs canonical JSONL under raw/green/{license_pool}/{target_id}
  - Caches tarballs under raw/yellow/quarantine/{target_id}/_cache when enabled
  - Uses globals.screening defaults when text_processing_defaults are missing
...
def main() -> None:
    ...
    ap.add_argument("--raw-root", default="/data/safety/raw")
    ap.add_argument("--target-id", default="pmc_oa_fulltext", help="Target id to use for raw output folder")
    ...
    ap.add_argument("--log-dir", default="/data/safety/_logs")
    args = ap.parse_args()

    targets_path = Path(args.targets).expanduser().resolve()
    raw_root = raw_root_from_targets_yaml(targets_path, Path(args.raw_root).expanduser().resolve())
    ...
    out_root = raw_root / "green" / "permissive" / args.target_id
    shards_root = out_root / "shards"
    manifests_dir = out_root / "_manifests"
    ensure_dir(shards_root)
    ensure_dir(manifests_dir)

    cache_dir = raw_root / "yellow" / "quarantine" / args.target_id / "_cache" if args.enable_cache else None
    if cache_dir:
        ensure_dir(cache_dir)
