...
1. Classify targets and snapshot license evidence (`pipeline_driver.py`).
...
6. Build catalogs over the new layout (`catalog_builder.py`).
...
| Stage | Script | Notes |
| --- | --- | --- |
| Classify | `pipeline_driver.py` | Emits GREEN/YELLOW/RED queues; adds routing for difficulty assignment. |
...
| Difficulty | `difficulty_worker.py` | Final light screen + safety rubric difficulty assignment; writes difficulty shards and ledger. |
