...
Builds a lightweight catalog for the v2 chemistry pipeline layout. Summaries are
...
def build_catalog(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    combined_root = Path(g.get("combined_root", "/data/chem/combined"))
    final_root = Path(g.get("final_root", "/data/chem/final"))
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="targets_chem.yaml")
