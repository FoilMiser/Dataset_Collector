...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    chem_routing = (target.get("chem_routing", {}) or {})
    math_routing = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or chem_routing.get("subject") or math_routing.get("subject") or "chem"
    domain = routing.get("domain") or chem_routing.get("domain") or math_routing.get("domain") or "misc"
    category = routing.get("category") or chem_routing.get("category") or math_routing.get("category") or "misc"
    level = coerce_int(
        routing.get("level"),
        coerce_int(chem_routing.get("level"), coerce_int(math_routing.get("level"), 5)),
    )
    granularity = routing.get("granularity") or chem_routing.get("granularity") or math_routing.get("granularity") or "target"
    ...
    return {
        ...
        "confidence": routing.get("confidence") or chem_routing.get("confidence"),
        "reason": routing.get("reason") or chem_routing.get("reason"),
    }
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_chem.yaml (v0.8)")
    ...
    for t in targets:
        ...
        evaluation = {
            ...
            "chem_routing": {
                ...
                "level": cr.get("level"),
                "granularity": cr.get("granularity"),
            },
            "routing": routing,
        }
