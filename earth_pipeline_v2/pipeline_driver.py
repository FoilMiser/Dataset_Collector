...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    earth_routing = (target.get("earth_routing", {}) or {})
    legacy_math = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or earth_routing.get("subject") or legacy_math.get("subject") or "earth"
    domain = routing.get("domain") or earth_routing.get("domain") or legacy_math.get("domain")
    category = routing.get("category") or earth_routing.get("category") or legacy_math.get("category")
    level = coerce_int(routing.get("level"), coerce_int(earth_routing.get("level", legacy_math.get("level"))))
    granularity = routing.get("granularity") or earth_routing.get("granularity") or legacy_math.get("granularity") or "target"
...
def load_license_map(path: Path) -> LicenseMap:
    ...
    gating = m.get("gating", {})
    profiles = m.get("license_profiles", {}) or m.get("profiles", {})
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"earth-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def generate_dry_run_report(
    ...
    lines = [
        "=" * 70,
        "EARTH SYSTEMS PIPELINE — DRY-RUN SUMMARY REPORT",
        f"Generated: {utc_now()}",
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_earth.yaml (v0.8)")
