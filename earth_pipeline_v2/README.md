# Earth Systems Corpus Pipeline (v2)

A safety-first pipeline for building an ethical environmental and Earth systems corpus. Version 2 restructures the Earth pipeline to mirror the v2 stage flow:
...
The recommended roots live in `../pipelines/targets/targets_earth.yaml -> globals`:
...
dc pipeline earth -- --targets ../pipelines/targets/targets_earth.yaml --stage classify --no-fetch

# Acquire GREEN and YELLOW (execute downloads)
dc run --pipeline earth --stage acquire --allow-data-root -- --queue /data/earth/_queues/green_pipeline.jsonl --bucket green --targets-yaml ../pipelines/targets/targets_earth.yaml --execute
dc run --pipeline earth --stage acquire --allow-data-root -- --queue /data/earth/_queues/yellow_pipeline.jsonl --bucket yellow --targets-yaml ../pipelines/targets/targets_earth.yaml --execute

# Screen, merge, catalog
dc run --pipeline earth --stage yellow_screen --allow-data-root -- --queue /data/earth/_queues/yellow_pipeline.jsonl --targets ../pipelines/targets/targets_earth.yaml --execute
dc run --pipeline earth --stage merge --allow-data-root -- --targets ../pipelines/targets/targets_earth.yaml --execute
dc catalog-builder --pipeline earth --allow-data-root -- --targets ../pipelines/targets/targets_earth.yaml --output /data/earth/_catalogs/catalog.json
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `../pipelines/targets/targets_earth.yaml -> globals.screening` and per-target `yellow_screen` overrides.
