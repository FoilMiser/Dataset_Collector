...
The recommended roots live in `targets_earth.yaml -> globals`:
...
./run_pipeline.sh --targets targets_earth.yaml --stage classify

# Acquire GREEN and YELLOW (execute downloads)
./run_pipeline.sh --targets targets_earth.yaml --stage acquire_green --execute
./run_pipeline.sh --targets targets_earth.yaml --stage acquire_yellow --execute

# Screen, merge, difficulty, catalog
./run_pipeline.sh --targets targets_earth.yaml --stage screen_yellow --execute
./run_pipeline.sh --targets targets_earth.yaml --stage merge --execute
./run_pipeline.sh --targets targets_earth.yaml --stage difficulty --execute
./run_pipeline.sh --targets targets_earth.yaml --stage catalog
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `targets_earth.yaml -> globals.screening` and per-target `yellow_screen` overrides.
