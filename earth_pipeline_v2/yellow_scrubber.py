"""
Earth pipeline helper for YELLOW bucket planning and manual review prep.

This script focuses on planning and human triage; transforms occur in workers.
...
Usage examples:
    python yellow_scrubber.py --targets targets_earth.yaml
    python yellow_scrubber.py --targets targets_earth.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_earth.yaml")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/earth/_queues")).expanduser()
