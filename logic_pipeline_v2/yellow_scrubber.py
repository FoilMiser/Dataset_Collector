"""
Logic pipeline helper for YELLOW bucket planning and manual review prep.

This script does **not** perform logic-specific transforms yet (future workers
handle HF filters, formal chunking, and PDF extraction). Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_logic.yaml
    python yellow_scrubber.py --targets targets_logic.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Logic pipeline YELLOW review helper")
    ap.add_argument("--targets", required=True, help="Path to targets_logic.yaml")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/logic/_queues")).expanduser()
    ...
    print("Logic YELLOW queue summary")
