...
#
# Wrapper script for the Logic Corpus Pipeline v2. It follows the
...
print(cfg.get("globals", {}).get("queues_root", "/data/logic/_queues"))
PY
)
CATALOGS_ROOT=$(python - << PY
import yaml
cfg = yaml.safe_load(open("${TARGETS}")) or {}
print(cfg.get("globals", {}).get("catalogs_root", "/data/logic/_catalogs"))
PY
)
