...
def resolve_roots(cfg: Dict[str, Any]) -> Roots:
    ...
    return Roots(
        raw_root=Path(g.get("raw_root", "/data/logic/raw")),
        screened_root=Path(g.get("screened_yellow_root", "/data/logic/screened_yellow")),
        manifests_root=Path(g.get("manifests_root", "/data/logic/_manifests")),
        ledger_root=Path(g.get("ledger_root", "/data/logic/_ledger")),
        pitches_root=Path(g.get("pitches_root", "/data/logic/_pitches")),
    )
...
def canonical_record(raw: Dict[str, Any], text: str, target_id: str, license_profile: str, license_spdx: Optional[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "routing": raw.get("routing") or raw.get("logic_routing") or raw.get("route") or {},
