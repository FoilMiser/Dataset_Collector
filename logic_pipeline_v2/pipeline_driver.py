...
Reads:
  - targets_logic.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logic_routing = (target.get("logic_routing", {}) or {})

    subject = routing.get("subject") or logic_routing.get("subject") or "logic"
    domain = routing.get("domain") or logic_routing.get("domain")
    category = routing.get("category") or logic_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(logic_routing.get("level")))
    granularity = routing.get("granularity") or logic_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"logic-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_logic.yaml (v0.8)")
    ...
    for t in targets:
        ...
        # Output pool selection + logic routing (difficulty-aware download paths)
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "logic_domain": mr.get("domain"),
            "logic_category": mr.get("category"),
            "difficulty_level": mr.get("level"),
            "logic_granularity": mr.get("granularity"),
            # Generic routing (v2)
