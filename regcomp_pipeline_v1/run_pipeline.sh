...
#
# Single wrapper script for the Regulation & Compliance Corpus Pipeline.
...
#   yellow    - Process YELLOW targets (scrubbers/transforms)
#   pmc       - Download and chunk PMC articles (legacy hook; replace with regcomp workers)
...
usage() {
    cat << EOF
Regulation & Compliance Corpus Pipeline v${VERSION}
...
# Compute queues root once for downstream stages
QUEUES_ROOT=$(python -c "import yaml, pathlib; p=pathlib.Path('$TARGETS'); print((yaml.safe_load(p.read_text()) or {}).get('globals', {}).get('queues_root', '/data/regcomp/_queues'))")

# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Regulation & Compliance Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_review() {
  ...
  if [[ ! -f "$QUEUE_FILE" ]]; then
    echo -e "${YELLOW}Queue not found (did classify run yet?):${NC} $QUEUE_FILE"
    return 0
  fi
  python3 review_queue.py --queue "$QUEUE_FILE" list --limit 50 || true
}
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/regcomp/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/regcomp/_catalogs'))")
