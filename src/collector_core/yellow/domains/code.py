...
LICENSE_PATTERNS = {
    "MIT": re.compile(r"(?:MIT License|Permission is hereby granted,?\s+free of charge)", re.IGNORECASE),
    "Apache-2.0": re.compile(r"(?:Apache License.*Version 2\.0|Licensed under the Apache License)", re.IGNORECASE),
    "GPL-3.0": re.compile(r"(?:GNU General Public License.*(?:version\s+)?3|GPLv3)", re.IGNORECASE),
    "GPL-2.0": re.compile(r"(?:GNU General Public License.*(?:version\s+)?2|GPLv2)", re.IGNORECASE),
    "BSD-3-Clause": re.compile(r"(?:BSD 3-Clause|three conditions|Redistributions? of source code)", re.IGNORECASE),
}
...
def extract_license_info(text: str, max_lines: int = 100) -> dict[str, Any]:
    ...
    lines = text.split("\n")[:max_lines]
    header = "\n".join(lines)
    result: dict[str, Any] = {"has_license_header": False, "detected_spdx": None, "detected_license": None, "confidence": 0.0}
...
def detect_secrets(text: str) -> list[dict[str, Any]]:
    ...
    for pattern, secret_type in SECRET_PATTERNS:
        for match in pattern.finditer(text):
            line_num = text.count("\n", 0, match.start()) + 1
            findings.append({"type": secret_type, "line": line_num})
    return findings
...
def detect_malware_patterns(text: str) -> list[dict[str, Any]]:
    ...
    for pattern in MALWARE_PATTERNS:
        for match in pattern.finditer(text):
            line_num = text.count("\n", 0, match.start()) + 1
            findings.append({"pattern": pattern.pattern[:50], "line": line_num, "matched": match.group(0)[:50]})
    return findings
...
def filter_record(raw: dict[str, Any], ctx: DomainContext) -> FilterDecision:
    ...
    text = raw.get("content", "") or raw.get("code", "") or raw.get("text", "") or ""

    malware_findings = detect_malware_patterns(text)
    if malware_findings:
        return FilterDecision(
            allow=False, reason="malware_pattern_detected",
            extra={"rejection_type": "security", "malware_findings": malware_findings[:5]},
        )
    ...
    if secret_findings:
        decision.reason = (decision.reason or "") + ("; " if decision.reason else "") + "secrets_detected"
