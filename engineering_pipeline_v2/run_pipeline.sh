#!/usr/bin/env bash
#
# run_pipeline.sh (engineering v2)
...
usage() {
  cat << EOF
Engineering Corpus Pipeline v${VERSION}

Required:
  --targets FILE          Path to targets_engineering.yaml
...
echo -e "${BLUE}Engineering Corpus Pipeline v${VERSION}${NC}"
