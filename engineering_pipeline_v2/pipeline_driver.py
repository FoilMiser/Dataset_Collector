...
Reads:
  - targets_engineering.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    engineering_routing = (target.get("engineering_routing", {}) or {})

    subject = routing.get("subject") or engineering_routing.get("subject") or "engineering"
    domain = routing.get("domain") or engineering_routing.get("domain")
    category = routing.get("category") or engineering_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(engineering_routing.get("level")))
    granularity = routing.get("granularity") or engineering_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"engineering-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_engineering.yaml (v0.8)")
    ...
    for t in targets:
        ...
        # Output pool selection + engineering routing (difficulty-aware download paths)
        ...
        mr = t.get("engineering_routing", {}) or {}
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "engineering_domain": mr.get("domain"),
            "engineering_category": mr.get("category"),
            "difficulty_level": mr.get("level"),
            "engineering_granularity": mr.get("granularity"),
            # Generic routing (v2)
