...
This script does **not** perform engineering-specific transforms yet (future workers
handle targeted filters and chunking). Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_engineering.yaml
    python yellow_scrubber.py --targets targets_engineering.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_engineering.yaml")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/engineering/_queues")).expanduser()
