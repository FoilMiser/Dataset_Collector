...
Usage:
    python catalog_builder.py --targets targets.yaml --output /data/nlp_gov/_catalogs/global_catalog.json
...
def main() -> None:
    ...
    permissive_root = Path(pools_cfg.get("permissive", "/data/nlp_gov/pools/permissive")).expanduser()
    copyleft_root = Path(pools_cfg.get("copyleft", "/data/nlp_gov/pools/copyleft")).expanduser()
    quarantine_root = Path(pools_cfg.get("quarantine", "/data/nlp_gov/pools/quarantine")).expanduser()
    manifests_root = Path(globals_cfg.get("manifests_root", "/data/nlp_gov/_manifests")).expanduser()
