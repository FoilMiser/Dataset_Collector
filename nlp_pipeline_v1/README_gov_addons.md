...
- They also include an `output.pool` field, but **your v0.7 pipeline_driver currently does not
  propagate target.output.pool into the download queue**. If you notice downloads going into the
  quarantine pool, patch either:
  (A) pipeline_driver.py to emit `output_pool`, or
  (B) download_worker.py to look up `output.pool` from targets.yaml when `output_pool` is missing.

If you want, I can generate a small, safe patch for (A) or (B) as a separate file.
