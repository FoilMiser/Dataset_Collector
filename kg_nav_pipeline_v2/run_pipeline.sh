...
#
# Wrapper script for the KG & Navigation Corpus Pipeline v2.
#
# Usage examples:
...
usage() {
  cat << 'EOM'
KG & Navigation Corpus Pipeline v${VERSION}
...
print(cfg.get("globals", {}).get("queues_root", "/data/kg_nav/_queues"))
PY
)
CATALOGS_ROOT=$(python - << PY
import yaml
cfg = yaml.safe_load(open("${TARGETS}")) or {}
print(cfg.get("globals", {}).get("catalogs_root", "/data/kg_nav/_catalogs"))
PY
)
