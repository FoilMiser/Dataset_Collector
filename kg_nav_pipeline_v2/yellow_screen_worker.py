...
def canonical_record(raw: Dict[str, Any], text: str, target_id: str, license_profile: str, license_spdx: Optional[str], routing_hint: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    record_id = str(raw.get("record_id") or raw.get("id") or sha256_text(f"{target_id}:{text}"))
    content_hash = sha256_text(text)
    source = raw.get("source", {}) or {}
    routing = raw.get("routing") or raw.get("nav_routing") or raw.get("kg_nav_routing") or raw.get("route") or routing_hint or {}
    return {
        "record_id": record_id,
        "text": text,
        "source": {
    ...
    for pool in pools:
        ...
        for file_path in iter_raw_files(raw_dir):
            for raw in read_jsonl(file_path):
                ...
                routing_hint = queue_row.get("routing") or queue_row.get("nav_routing") or target_cfg.get("nav_routing") or target_cfg.get("routing")
                rec = canonical_record(raw, text, target_id, license_profile, lic, routing_hint)
                passed += 1
                if execute:
