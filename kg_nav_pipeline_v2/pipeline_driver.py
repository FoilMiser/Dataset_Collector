...
Reads:
  - targets_kg_nav.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    nav_routing = (target.get("nav_routing", {}) or target.get("kg_nav_routing", {}) or {})

    subject = routing.get("subject") or nav_routing.get("subject") or "kg_nav"
    domain = routing.get("domain") or nav_routing.get("domain")
    category = routing.get("category") or nav_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(nav_routing.get("level")))
    granularity = routing.get("granularity") or nav_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"kg-nav-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def generate_dry_run_report(
    ...
    lines.extend([
        ...
        "  3. Run yellow_screen_worker.py, merge_worker.py, and difficulty_worker.py (or use run_pipeline.sh stages)",
        "",
        "=" * 70,
    ])
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_kg_nav.yaml (v0.8)")
    ...
    for t in targets:
        ...
        # Output pool selection + navigation routing (difficulty-aware download paths)
        ...
        nav_routing = t.get("nav_routing", {}) or t.get("kg_nav_routing", {}) or {}
        ...
        evaluation = {
            ...
            "nav_routing": {
                "domain": nav_routing.get("domain"),
                "category": nav_routing.get("category"),
                "level": nav_routing.get("level"),
                "granularity": nav_routing.get("granularity"),
            },
            "routing": routing,
        }
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "nav_domain": nav_routing.get("domain"),
            "nav_category": nav_routing.get("category"),
            "difficulty_level": nav_routing.get("level"),
            "nav_granularity": nav_routing.get("granularity"),
            # Generic routing (v2)
