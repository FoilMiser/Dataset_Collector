#!/usr/bin/env python3
"""
pipeline_driver.py (v2.0 — kg_nav)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kg_routing = (target.get("kg_routing", {}) or {})
    legacy_math = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or kg_routing.get("subject") or legacy_math.get("subject") or "kg_nav"
    domain = routing.get("domain") or kg_routing.get("domain") or legacy_math.get("domain")
    category = routing.get("category") or kg_routing.get("category") or legacy_math.get("category")
    level = coerce_int(routing.get("level"), coerce_int(kg_routing.get("level"), coerce_int(legacy_math.get("level"))))
    granularity = routing.get("granularity") or kg_routing.get("granularity") or legacy_math.get("granularity") or "target"
    ...
    return {
        ...
        "confidence": routing.get("confidence") or kg_routing.get("confidence"),
        "reason": routing.get("reason") or kg_routing.get("reason"),
    }
