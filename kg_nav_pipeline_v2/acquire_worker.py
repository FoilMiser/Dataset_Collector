...
def handle_figshare(ctx: AcquireContext, row: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    ...
    download = row.get("download", {}) or {}
    urls = download.get("urls") or download.get("articles") or []
    if not urls:
    ...
    for url in urls:
        ...
        for idx, fmeta in enumerate(files[:max_files]):
            ...
            info = _http_download_with_resume(ctx, dl_url, out_path, expected_size=size)
...
def handle_s3_sync(ctx: AcquireContext, row: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    urls = (row.get("download", {}) or {}).get("urls") or []
    if not urls:
        return [{"status": "noop", "reason": "no s3 urls"}]
    ensure_dir(out_dir)
    results: List[Dict[str, Any]] = []
    no_sign = bool((row.get("download", {}) or {}).get("no_sign_request", True))
    extra = (row.get("download", {}) or {}).get("extra_args") or []
    for url in urls:
        if not ctx.mode.execute:
...
def handle_manual_email_link(ctx: AcquireContext, row: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    dl = row.get("download", {}) or {}
    urls = dl.get("urls") or []
    note = dl.get("notes") or "Manual request required"
    ensure_dir(out_dir)
    status = "planned" if not ctx.mode.execute else "manual"
    return [{"status": status, "note": note, "urls": urls, "dest": str(out_dir)}]
...
STRATEGY_HANDLERS = {
    ...
    "manual_email_link": handle_manual_email_link,
...
def load_roots(targets_path: Optional[Path], overrides: argparse.Namespace) -> Roots:
    ...
    raw_root = Path(overrides.raw_root or g.get("raw_root", "/data/kg_nav/raw"))
    manifests_root = Path(overrides.manifests_root or g.get("manifests_root", "/data/kg_nav/_manifests"))
    logs_root = Path(overrides.logs_root or g.get("logs_root", "/data/kg_nav/_logs"))
...
def main() -> None:
    ...
    ap.add_argument("--targets-yaml", default=None, help="Path to targets_kg_nav.yaml for roots")
