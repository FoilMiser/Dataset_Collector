...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    three_d_routing = (target.get("three_d_routing", {}) or {})

    subject = routing.get("subject") or three_d_routing.get("subject") or "3d"
    domain = routing.get("domain") or three_d_routing.get("domain")
    category = routing.get("category") or three_d_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(three_d_routing.get("level")))
    granularity = routing.get("granularity") or three_d_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"3d-modeling-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_3d.yaml (v0.8)")
    ...
    for t in targets:
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "three_d_domain": td.get("domain"),
            "three_d_category": td.get("category"),
            "difficulty_level": td.get("level"),
            "three_d_granularity": td.get("granularity"),
            # Generic routing (v2)
