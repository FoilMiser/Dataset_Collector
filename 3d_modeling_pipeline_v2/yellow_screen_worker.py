...
behavior. Outputs:
  ...
  - _pitches/yellow_pitched.jsonl (pitched samples)
...
def resolve_roots(cfg: Dict[str, Any]) -> Roots:
    ...
    return Roots(
        raw_root=Path(g.get("raw_root", "/data/3d/raw")),
        screened_root=Path(g.get("screened_yellow_root", "/data/3d/screened_yellow")),
        manifests_root=Path(g.get("manifests_root", "/data/3d/_manifests")),
...
def canonical_record(
    ...
    mesh_meta: Optional[Dict[str, Any]] = None,
    ...
    return {
        ...
        "routing": raw.get("routing") or raw.get("three_d_routing") or raw.get("route") or {},
        "hash": {"content_sha256": content_hash or sha256_text(text)},
        "assets": assets or [],
        "mesh": mesh_meta or {},
    }


def maybe_process_mesh(asset_path: Path, output_root: Path, license_spdx: Optional[str]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    try:
        import mesh_worker
    except Exception:
        return None, None
    ...
    try:
        metadata, status = mesh_worker.process_file(
            asset_path,
            output_root,
            source_id=asset_path.stem,
            license_spdx=license_spdx or "",
            attribution=None,
            creator=None,
            generate_thumbnails=True,
            generate_point_clouds=False,
        )
        if not metadata:
            return None, None
        return metadata, metadata.get("sha256_normalized") or metadata.get("sha256_original")
    except Exception:
        return None, None
    ...
    for pool in pools:
        ...
        for file_path in iter_raw_files(raw_dir):
            for raw in read_jsonl(file_path):
                ...
                mesh_meta: Optional[Dict[str, Any]] = None
                content_hash = None
                asset_ref = raw.get("asset_path")
                if asset_ref:
                    asset_path = (raw_dir / asset_ref).resolve()
                    if asset_path.exists():
                        file_hash = sha256_file(asset_path)
                        content_hash = file_hash or content_hash
                        try:
                            rel_path = asset_path.relative_to(raw_dir)
                        except ValueError:
                            rel_path = asset_path.name
                        assets.append({"path": str(rel_path), "sha256": file_hash})
                        mesh_meta, mesh_hash = maybe_process_mesh(asset_path, raw_dir / "_mesh", lic)
                        if mesh_hash:
                            content_hash = mesh_hash

                if not text:
                    if assets:
                        text = f"3d asset {assets[0]['path']}"
                    else:
                        pitched += 1
                        if execute:
                            append_jsonl(roots.ledger_root / "yellow_pitched.jsonl", [{"target_id": target_id, "reason": "no_text", "sample": raw}])
                            append_jsonl(roots.pitches_root / "yellow_pitched.jsonl", [{"target_id": target_id, "reason": "no_text", "sample": raw}])
                        continue
                if len(text) < screen_cfg.min_chars or len(text) > screen_cfg.max_chars:
                    ...
                    if execute:
                        ...
                        append_jsonl(roots.pitches_root / "yellow_pitched.jsonl", [{"target_id": target_id, "reason": "length_bounds", "sample_id": raw.get("id")}])
                    continue
                if screen_cfg.require_record_license and not lic:
                    ...
                    if execute:
                        ...
                        append_jsonl(roots.pitches_root / "yellow_pitched.jsonl", [{"target_id": target_id, "reason": "missing_record_license", "sample_id": raw.get("id")}])
                    continue
                if lic and screen_cfg.allow_spdx and lic not in screen_cfg.allow_spdx:
                    ...
                    if execute:
                        ...
                        append_jsonl(roots.pitches_root / "yellow_pitched.jsonl", [{"target_id": target_id, "reason": "license_not_allowlisted", "license": lic, "sample_id": raw.get("id")}])
                    continue
                if contains_deny(text, screen_cfg.deny_phrases):
                    ...
                    if execute:
                        ...
                        append_jsonl(roots.pitches_root / "yellow_pitched.jsonl", [{"target_id": target_id, "reason": "deny_phrase", "sample_id": raw.get("id")}])
                    continue
                license_profile = str(raw.get("license_profile") or queue_row.get("license_profile") or pool or "quarantine")
                rec = canonical_record(raw, text, target_id, license_profile, lic, assets=assets, mesh_meta=mesh_meta, content_hash=content_hash)
                passed += 1
                if execute:
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_3d.yaml")
