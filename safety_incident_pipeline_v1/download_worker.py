...
def http_download_with_resume(ctx: DownloaderContext, url: str, out_path: Path, 
    ...
    for attempt in range(ctx.retry.max_attempts):
        ...
        try:
            headers = {"User-Agent": f"safety-incident-corpus-downloader/{VERSION}"}
            if start_byte > 0:
...
def handle_zenodo(ctx: DownloaderContext, target: Dict[str, Any], out_dir: Path) -> List[Dict[str, Any]]:
    ...
    for attempt in range(ctx.retry.max_attempts):
        try:
            r = requests.get(api_url, timeout=60, headers={"User-Agent": f"safety-incident-corpus/{VERSION}"})
            r.raise_for_status()
            meta = r.json()
            break
        except Exception as e:
...
def main() -> None:
    ...
    ap.add_argument("--pools-root", default="/data/safety/pools", help="Fallback pools root")
    ap.add_argument("--logs-dir", default="/data/safety/_logs", help="Logs directory")
