...
#
# Single wrapper script for the Safety Incident & Compliance Corpus Pipeline.
...
#   yellow    - Process YELLOW targets (e.g., PII scrub/allowlist helpers)
#   pmc       - Download and chunk PMC articles (if used)
...
usage() {
    cat << EOF
Safety Incident & Compliance Corpus Pipeline v${VERSION}
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Safety Incident & Compliance Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/safety/_queues'))")
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/safety/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/safety/_catalogs'))")
