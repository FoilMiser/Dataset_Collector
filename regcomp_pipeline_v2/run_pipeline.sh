...
#
# Wrapper script for the Regulation & Compliance Corpus Pipeline v2.
#
# Usage examples:
...
Required:
  --targets FILE          Path to targets_regcomp.yaml
...
print(cfg.get("globals", {}).get("queues_root", "/data/regcomp/_queues"))
PY
)
CATALOGS_ROOT=$(python - << PY
import yaml
cfg = yaml.safe_load(open("${TARGETS}")) or {}
print(cfg.get("globals", {}).get("catalogs_root", "/data/regcomp/_catalogs"))
PY
)
