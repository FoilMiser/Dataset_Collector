#!/usr/bin/env python3
"""
yellow_scrubber.py (regcomp pipeline v0.9)

Stage-2 transformations for YELLOW bucket datasets (quarantine -> permissive),
plus record-level allowlist planning for the regulatory compliance domain.

This module imports shared implementation from collector_core.yellow_scrubber_base,
providing regcomp-specific defaults and user-agent strings.
...
# Regcomp-specific user agent prefix
_USER_AGENT_PREFIX = "regcomp-corpus-scrubber"


def fetch_text_with_fallback(urls: list[str], timeout_s: int = 30) -> tuple[str, str]:
    """Regcomp-specific wrapper with appropriate user agent."""
...
def main() -> None:
    """CLI entry point for regcomp yellow scrubber."""
    ...
    logs_dir = Path("/data/regcomp/_logs").expanduser()
    ensure_dir(logs_dir)
