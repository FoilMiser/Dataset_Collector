...
The recommended roots live in `targets_regcomp.yaml -> globals`:
...
| Stage | Script | Notes |
| --- | --- | --- |
| Classify | `pipeline_driver.py` | Emits GREEN/YELLOW/RED queues; adds routing metadata for difficulty assignment. |
...
./run_pipeline.sh --targets targets_regcomp.yaml --stage classify

# Acquire GREEN and YELLOW (execute downloads)
./run_pipeline.sh --targets targets_regcomp.yaml --stage acquire_green --execute
./run_pipeline.sh --targets targets_regcomp.yaml --stage acquire_yellow --execute

# Screen, merge, difficulty, catalog
./run_pipeline.sh --targets targets_regcomp.yaml --stage screen_yellow --execute
./run_pipeline.sh --targets targets_regcomp.yaml --stage merge --execute
./run_pipeline.sh --targets targets_regcomp.yaml --stage difficulty --execute
./run_pipeline.sh --targets targets_regcomp.yaml --stage catalog
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `targets_regcomp.yaml -> globals.screening` and per-target `yellow_screen` overrides.
