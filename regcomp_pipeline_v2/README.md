# Regulation & Compliance Corpus Pipeline (v2)

A safety-first pipeline for building a regulation & compliance training corpus. Version 2 aligns the regcomp workflow with the **math_pipeline_v2** stage order:
...
The recommended roots live in `../pipelines/targets/targets_regcomp.yaml -> globals`:
...
| Stage | Script | Notes |
| --- | --- | --- |
| Classify | `pipeline_driver.py` | Emits GREEN/YELLOW/RED queues; adds routing metadata for downstream sorting. |
...
dc pipeline regcomp -- --targets ../pipelines/targets/targets_regcomp.yaml --stage classify --no-fetch

# Acquire GREEN and YELLOW (execute downloads)
dc run --pipeline regcomp --stage acquire --allow-data-root -- --queue /data/regcomp/_queues/green_pipeline.jsonl --bucket green --targets-yaml ../pipelines/targets/targets_regcomp.yaml --execute
dc run --pipeline regcomp --stage acquire --allow-data-root -- --queue /data/regcomp/_queues/yellow_pipeline.jsonl --bucket yellow --targets-yaml ../pipelines/targets/targets_regcomp.yaml --execute

# Screen, merge, catalog
dc run --pipeline regcomp --stage yellow_screen --allow-data-root -- --queue /data/regcomp/_queues/yellow_pipeline.jsonl --targets ../pipelines/targets/targets_regcomp.yaml --execute
dc run --pipeline regcomp --stage merge --allow-data-root -- --targets ../pipelines/targets/targets_regcomp.yaml --execute
dc catalog-builder --pipeline regcomp --allow-data-root -- --targets ../pipelines/targets/targets_regcomp.yaml --output /data/regcomp/_catalogs/catalog.json
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `../pipelines/targets/targets_regcomp.yaml -> globals.screening` and per-target `yellow_screen` overrides.
