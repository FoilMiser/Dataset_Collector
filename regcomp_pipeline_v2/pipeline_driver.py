...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    regcomp_routing = (target.get("regcomp_routing", {}) or {})

    subject = routing.get("subject") or regcomp_routing.get("subject") or "regcomp"
    domain = routing.get("domain") or regcomp_routing.get("domain")
    category = routing.get("category") or regcomp_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(regcomp_routing.get("level")))
    granularity = routing.get("granularity") or regcomp_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"regcomp-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_regcomp.yaml (v0.8)")
