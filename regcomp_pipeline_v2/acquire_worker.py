...
def load_roots(targets_path: Optional[Path], overrides: argparse.Namespace) -> Roots:
    ...
    raw_root = Path(overrides.raw_root or g.get("raw_root", "/data/regcomp/raw"))
    manifests_root = Path(overrides.manifests_root or g.get("manifests_root", "/data/regcomp/_manifests"))
    logs_root = Path(overrides.logs_root or g.get("logs_root", "/data/regcomp/_logs"))
...
def main() -> None:
    ...
    ap.add_argument("--targets-yaml", default=None, help="Path to targets_regcomp.yaml for roots")
