...
# Resolve key roots from targets.yaml with physics-friendly defaults
...
print(Path(globals_cfg.get("queues_root", "/data/physics/_queues")).expanduser())
PY
)
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/physics/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/physics/_catalogs'))")
