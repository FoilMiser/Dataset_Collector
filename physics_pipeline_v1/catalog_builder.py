...
Usage:
  python catalog_builder.py --targets targets.yaml --output /data/physics/_catalogs/global_catalog.json
...
def main() -> None:
    ...
    permissive_root = Path(pools_cfg.get("permissive", "/data/physics/pools/permissive")).expanduser()
    copyleft_root = Path(pools_cfg.get("copyleft", "/data/physics/pools/copyleft")).expanduser()
    quarantine_root = Path(pools_cfg.get("quarantine", "/data/physics/pools/quarantine")).expanduser()
    manifests_root = Path(globals_cfg.get("manifests_root", "/data/physics/_manifests")).expanduser()
