"""
Econ/Stats/Decision pipeline helper for YELLOW bucket planning and manual review prep.

This script does **not** perform domain-specific transforms. Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_econ_stats_decision_v2.yaml
    python yellow_scrubber.py --targets targets_econ_stats_decision_v2.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Econ/Stats/Decision pipeline YELLOW review helper")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/econ/_queues")).expanduser()
    ...
    print("Econ/Stats/Decision YELLOW queue summary")
