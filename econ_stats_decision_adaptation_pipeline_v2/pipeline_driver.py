...
Reads:
  - targets_econ_stats_decision_v2.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    econ_routing = (target.get("econ_routing", {}) or {})

    subject = routing.get("subject") or econ_routing.get("subject") or math_routing.get("subject") or "econ"
    domain = routing.get("domain") or econ_routing.get("domain") or math_routing.get("domain")
    category = routing.get("category") or econ_routing.get("category") or math_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(econ_routing.get("level"), coerce_int(math_routing.get("level"))))
    granularity = routing.get("granularity") or econ_routing.get("granularity") or math_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"econ-stats-corpus/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_econ_stats_decision_v2.yaml (v0.8)")
    ...
    for t in targets:
        ...
        # Output pool selection + routing (difficulty-aware download paths)
        ...
        mr = t.get("math_routing", {}) or t.get("econ_routing", {}) or {}
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "routing_subject": routing.get("subject") or mr.get("subject"),
            "routing_domain": routing.get("domain") or mr.get("domain"),
            "routing_category": routing.get("category") or mr.get("category"),
            "routing_level": routing.get("level") or mr.get("level"),
            "routing_granularity": routing.get("granularity") or mr.get("granularity"),
            # Generic routing (v2)
