...
def resolve_roots(cfg: Dict[str, Any]) -> Roots:
    ...
    return Roots(
        combined_root=Path(g.get("combined_root", "/data/econ/combined")),
        final_root=Path(g.get("final_root", "/data/econ/final")),
...
def level_from_routing(diff_cfg: Dict[str, Any], routing: Dict[str, Any]) -> Optional[Tuple[int, int, int]]:
    subj = (routing or {}).get("subject") or "econ"
    ...
    if category and category in (dom_cfg.get("categories", {}) or {}):
        ...
        if not level_info:
            return None
        default_level = level_info.get("default")
        if default_level is None:
            return None
        min_level = int(level_info.get("min", default_level))
        max_level = int(level_info.get("max", default_level))
        return int(default_level), min_level, max_level
    return None
...
def keyword_route(diff_cfg: Dict[str, Any], text: str) -> Optional[Dict[str, Any]]:
    rules = ((diff_cfg.get("rule_sets", {}) or {}).get("global", {}) or {}).get("keyword_rules", []) or []
    low = text.lower()
    ...
    for rule in rules:
        ...
        if any(k.lower() in low for k in keywords):
            ...
            if not best or priority > best.get("priority", 0):
                best = {
...
def assign_difficulty(diff_cfg: Dict[str, Any], record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    routing_info = level_from_routing(diff_cfg, routing)
    if routing_info:
        default_level, min_level, max_level = routing_info
        bounded = clamp_level(int(default_level), min_level, max_level)
        return {"level": bounded, "method": "routing", "confidence": 0.7}
    kw_hit = keyword_route(diff_cfg, record.get("text", ""))
    if kw_hit:
        route = kw_hit.get("route") or {}
        if route and not record.get("routing"):
            record["routing"] = route
        kw_info = level_from_routing(diff_cfg, route)
        guess_level = (route.get("level") if isinstance(route, dict) else None) or (kw_info[0] if kw_info else None) or 5
        min_level = kw_info[1] if kw_info else 1
        max_level = kw_info[2] if kw_info else 10
        bounded = clamp_level(int(guess_level), min_level, max_level)
        return {"level": bounded, "method": "keyword_rule", "confidence": kw_hit.get("confidence", 0.6)}
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="targets_econ_stats_decision_v2.yaml")
