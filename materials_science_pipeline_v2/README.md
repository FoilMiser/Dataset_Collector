# Materials Science Corpus Pipeline (v2)

A safety-first pipeline for building a materials science training corpus. Version 2 follows the v2 stage flow:
...
The recommended roots live in `../pipelines/targets/targets_materials.yaml -> globals`:
...
dc pipeline materials_science -- --targets ../pipelines/targets/targets_materials.yaml --stage classify --no-fetch

# Acquire GREEN and YELLOW (execute downloads)
dc run --pipeline materials_science --stage acquire --allow-data-root -- --queue /data/materials_science/_queues/green_pipeline.jsonl --bucket green --targets-yaml ../pipelines/targets/targets_materials.yaml --execute
dc run --pipeline materials_science --stage acquire --allow-data-root -- --queue /data/materials_science/_queues/yellow_pipeline.jsonl --bucket yellow --targets-yaml ../pipelines/targets/targets_materials.yaml --execute

# Screen, merge, catalog
dc run --pipeline materials_science --stage yellow_screen --allow-data-root -- --queue /data/materials_science/_queues/yellow_pipeline.jsonl --targets ../pipelines/targets/targets_materials.yaml --execute
dc run --pipeline materials_science --stage merge --allow-data-root -- --targets ../pipelines/targets/targets_materials.yaml --execute
dc catalog-builder --pipeline materials_science --allow-data-root -- --targets ../pipelines/targets/targets_materials.yaml --output /data/materials_science/_catalogs/catalog.json
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `../pipelines/targets/targets_materials.yaml -> globals.screening` and per-target `yellow_screen` overrides.
