#!/usr/bin/env python3
"""
yellow_scrubber.py (materials_science pipeline v0.9)

Stage-2 transformations for YELLOW bucket datasets (quarantine -> permissive),
plus record-level allowlist planning for the materials science domain.

This module imports shared implementation from collector_core.yellow_scrubber_base,
providing materials-science-specific defaults and user-agent strings.
...
# Materials-science-specific user agent prefix
_USER_AGENT_PREFIX = "materials-corpus-scrubber"


def fetch_text_with_fallback(urls: list[str], timeout_s: int = 30) -> tuple[str, str]:
    """Materials-science-specific wrapper with appropriate user agent."""
...
def main() -> None:
    """CLI entry point for materials_science yellow scrubber."""
    ap = argparse.ArgumentParser(
        description=f"Yellow Scrubber v{TOOL_VERSION} (schema {SCHEMA_VERSION})"
    )
    ap.add_argument("--targets", required=True, help="targets_materials_science.yaml (v0.9)")
    ap.add_argument("--license-map", default=None)
    ap.add_argument("--field-schemas", default=None, help="field_schemas.yaml")
    ap.add_argument("--pools-root", default="/data/materials_science/pools")
    ...
    logs_dir = Path("/data/materials_science/_logs").expanduser()
    ensure_dir(logs_dir)
