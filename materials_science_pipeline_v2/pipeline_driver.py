...
Reads:
  - targets_materials.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    materials_routing = (target.get("materials_routing", {}) or {})
    legacy_math_routing = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or materials_routing.get("subject") or legacy_math_routing.get("subject") or "materials_science"
    domain = routing.get("domain") or materials_routing.get("domain") or legacy_math_routing.get("domain")
    category = routing.get("category") or materials_routing.get("category") or legacy_math_routing.get("category")
    level = coerce_int(
        routing.get("level"),
        coerce_int(materials_routing.get("level"), coerce_int(legacy_math_routing.get("level")))
    )
    granularity = routing.get("granularity") or materials_routing.get("granularity") or legacy_math_routing.get("granularity") or "target"
    ...
    return {
        ...
        "confidence": routing.get("confidence") or materials_routing.get("confidence"),
        "reason": routing.get("reason") or materials_routing.get("reason"),
    }
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"materials-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def generate_dry_run_report(
    ...
    lines = [
        "=" * 70,
        "MATERIALS SCIENCE CORPUS PIPELINE — DRY-RUN SUMMARY REPORT",
        f"Generated: {utc_now()}",
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_materials.yaml (v0.8)")
    ...
    for t in targets:
        ...
        # Output pool selection + materials routing (difficulty-aware download paths)
        ...
        mr = t.get("materials_routing", {}) or t.get("math_routing", {}) or {}
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "materials_domain": mr.get("domain"),
            "materials_category": mr.get("category"),
            "difficulty_level": mr.get("level"),
            "materials_granularity": mr.get("granularity"),
            # Generic routing (v2)
