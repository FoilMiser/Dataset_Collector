...
#
# Single wrapper script for the Logic Corpus Pipeline.
# Orchestrates all stages with sensible defaults.
#
# Usage:
#   ./run_pipeline.sh --targets targets_logic.yaml                    # Dry-run
#   ./run_pipeline.sh --targets targets_logic.yaml --execute          # Full run
#   ./run_pipeline.sh --targets targets_logic.yaml --stage download   # Specific stage
#
# Stages:
...
usage() {
    cat << EOF
Logic Corpus Pipeline v${VERSION}

Usage: $0 [OPTIONS]

Required:
  --targets FILE          Path to targets_logic.yaml
...
Examples:
  ...
  $0 --targets targets_logic.yaml

  # Execute with limits
  $0 --targets targets_logic.yaml --execute --limit-targets 3 --limit-files 5

  # Run only classification stage
  $0 --targets targets_logic.yaml --stage classify

  # Emit a YELLOW review plan
  $0 --targets targets_logic.yaml --stage yellow
...
QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/logic/_queues'))")

# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Logic Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_download() {
    ...
    # Get queues root from targets_logic.yaml
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/logic/_queues'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/logic/_catalogs'))")
