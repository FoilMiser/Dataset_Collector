...
#
# Single wrapper script for the Ethical Physics / NLP Corpus Pipeline.
...
usage() {
    cat << EOF
Ethical Physics / NLP Corpus Pipeline v${VERSION}
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Ethical Physics / NLP Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/nlp_gov/_queues'))")
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/nlp_gov/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/nlp_gov/_catalogs'))")
