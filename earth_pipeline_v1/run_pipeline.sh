...
#
# Single wrapper script for the Earth Systems Corpus Pipeline.
...
#   yellow    - Process YELLOW targets (Earth conditional targets; legacy PubChem/PMC hooks optional)
#   pmc       - Download and chunk PMC articles (compatibility hook, disabled by default)
...
usage() {
    cat << EOF
Earth Systems Corpus Pipeline v${VERSION}
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Earth Systems Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/earth/_queues'))")
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/earth/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/earth/_catalogs'))")
