...
- As of pipeline v1.0, `pipeline_driver.py` now **propagates `output.pool` into the green/yellow
  download queues** as `output_pool`, so downloads land in the intended pool (permissive/copyright/
  quarantine) without additional patches.
