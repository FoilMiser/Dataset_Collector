# Biology Corpus Pipeline Prototype (v1.0)

A safety-first **prototype** pipeline for building a biology-focused, ethics-first training corpus from open datasets and open-access literature, with strong emphasis on **license compliance, provenance tracking, and safe-by-default execution**. This folder adapts the chemistry pipeline with the guidance in `../BIOLOGY_PIPELINE_ADAPTATION_PLAN.md`, the biology-specific config pair (`targets.yaml` + `license_map.yaml` sourced from `biology_targets.yaml` and `bio_license_map.yaml`), and the government portal helpers documented in `README_gov_addons.md`.
...
# or:
python3 review_queue.py --queue /data/bio/_queues/yellow_pipeline.jsonl list
```
...
### Build catalog
```bash
python3 catalog_builder.py --targets targets.yaml --output /data/bio/_catalogs/global_catalog.json
```
