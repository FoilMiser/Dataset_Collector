...
#
# Single wrapper script for the Biology Corpus Pipeline.
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Biology Corpus Pipeline v${VERSION}${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/bio/_queues'))")
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/bio/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/bio/_catalogs'))")
