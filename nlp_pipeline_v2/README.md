# NLP Corpus Pipeline (v2)

A safety-first pipeline for building an ethical NLP training corpus. Version 2 restructures the NLP pipeline to match the v2 flow:
...
The recommended roots live in `../pipelines/targets/targets_nlp.yaml -> globals`. Queue and catalog paths can be overridden with `globals.queues_root` and `globals.catalogs_root`:
...
| Stage | Script | Notes |
| --- | --- | --- |
| Classify | `pipeline_driver.py` | Emits GREEN/YELLOW/RED queues; adds routing fields for downstream sorting. |
...
dc pipeline nlp -- --targets ../pipelines/targets/targets_nlp.yaml --stage classify --no-fetch

# Acquire GREEN and YELLOW (execute downloads)
dc run --pipeline nlp --stage acquire --allow-data-root -- --queue /data/nlp/_queues/green_pipeline.jsonl --bucket green --targets-yaml ../pipelines/targets/targets_nlp.yaml --execute
dc run --pipeline nlp --stage acquire --allow-data-root -- --queue /data/nlp/_queues/yellow_pipeline.jsonl --bucket yellow --targets-yaml ../pipelines/targets/targets_nlp.yaml --execute

# Screen, merge, catalog
dc run --pipeline nlp --stage yellow_screen --allow-data-root -- --queue /data/nlp/_queues/yellow_pipeline.jsonl --targets ../pipelines/targets/targets_nlp.yaml --execute
dc run --pipeline nlp --stage merge --allow-data-root -- --targets ../pipelines/targets/targets_nlp.yaml --execute
dc catalog-builder --pipeline nlp --allow-data-root -- --targets ../pipelines/targets/targets_nlp.yaml --output /data/nlp/_catalogs/catalog.json
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `../pipelines/targets/targets_nlp.yaml -> globals.screening` and per-target `yellow_screen` overrides.
