...
| Stage | Script | Notes |
| --- | --- | --- |
| Classify | `pipeline_driver.py` | Emits GREEN/YELLOW/RED queues; adds routing fields for difficulty mapping. |
...
./run_pipeline.sh --targets targets_nlp.yaml --stage acquire_yellow --execute

# Screen, merge, difficulty, catalog
./run_pipeline.sh --targets targets_nlp.yaml --stage screen_yellow --execute
./run_pipeline.sh --targets targets_nlp.yaml --stage merge --execute
./run_pipeline.sh --targets targets_nlp.yaml --stage difficulty --execute
./run_pipeline.sh --targets targets_nlp.yaml --stage catalog
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `targets_nlp.yaml -> globals.screening` and per-target `yellow_screen` overrides.
- Difficulty assignment happens in `difficulty_worker.py`, using `difficulties_nlp.yaml` routing defaults + heuristics.
