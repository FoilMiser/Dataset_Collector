...
def handle_github_release(ctx: AcquireContext, row: dict[str, Any], out_dir: Path) -> list[dict[str, Any]]:
    ...
    headers = {"User-Agent": f"nlp-corpus-acquire/{VERSION}"}
...
def load_roots(targets_path: Path | None, overrides: argparse.Namespace) -> Roots:
    ...
    logs_root = Path(overrides.logs_root or g.get("logs_root", "/data/nlp/_logs"))
...
def main() -> None:
    ...
    ap.add_argument("--targets-yaml", default=None, help="Path to targets_nlp.yaml for roots")
