...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    subject = routing.get("subject") or nlp_routing.get("subject") or "nlp"
    domain = routing.get("domain") or nlp_routing.get("domain") or "misc"
    category = routing.get("category") or nlp_routing.get("category") or "misc"
    level = coerce_int(routing.get("level"), coerce_int(nlp_routing.get("level")))
    granularity = routing.get("granularity") or nlp_routing.get("granularity") or "target"
    ...
    return {
        ...
        "confidence": routing.get("confidence") or nlp_routing.get("confidence"),
        "reason": routing.get("reason") or nlp_routing.get("reason"),
    }
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"nlp-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_nlp.yaml (v0.8)")
    ...
    for t in targets:
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "nlp_domain": mr.get("domain"),
            "nlp_category": mr.get("category"),
            "difficulty_level": mr.get("level"),
            "nlp_granularity": mr.get("granularity"),
            # Generic routing (v2)
