...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    bio_routing = (target.get("bio_routing", {}) or {})
    math_routing = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or bio_routing.get("subject") or math_routing.get("subject") or "biology"
    domain = routing.get("domain") or bio_routing.get("domain") or math_routing.get("domain")
    category = routing.get("category") or bio_routing.get("category") or math_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(bio_routing.get("level"), coerce_int(math_routing.get("level"))))
    granularity = routing.get("granularity") or bio_routing.get("granularity") or math_routing.get("granularity") or "target"
    ...
    return {
        ...
        "confidence": routing.get("confidence") or bio_routing.get("confidence"),
        "reason": routing.get("reason") or bio_routing.get("reason"),
    }
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"bio-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_biology.yaml (v0.8)")
