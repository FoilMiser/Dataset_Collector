...
Builds a lightweight catalog for the v2 biology pipeline layout. Summaries are
...
def build_catalog(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    screened_root = Path(g.get("screened_yellow_root", "/data/bio/screened_yellow"))
    combined_root = Path(g.get("combined_root", "/data/bio/combined"))
    final_root = Path(g.get("final_root", "/data/bio/final"))
    ledger_root = Path(g.get("ledger_root", "/data/bio/_ledger"))
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="targets_biology.yaml")
