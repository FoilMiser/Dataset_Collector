...
def resolve_roots(cfg: dict[str, Any]) -> Roots:
    ...
    return Roots(
        ...
        manifests_root=Path(g.get("manifests_root", "/data/bio/_manifests")),
        ledger_root=Path(g.get("ledger_root", "/data/bio/_ledger")),
        pitches_root=Path(g.get("pitches_root", "/data/bio/_pitches")),
    )
...
def canonical_record(raw: dict[str, Any], text: str, target_id: str, license_profile: str, license_spdx: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "routing": raw.get("routing") or raw.get("bio_routing") or raw.get("math_routing") or raw.get("route") or {},
