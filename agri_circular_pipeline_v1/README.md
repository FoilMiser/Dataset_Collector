# Agriculture + Circular Bioeconomy Pipeline (v1.0 adaptation)

A safety-first **prototype** pipeline for building an agriculture, waste-stream, and circular bioeconomy training corpus, adapted from the chemistry pipeline. It keeps the same emphasis on **license compliance, provenance tracking, and safe-by-default execution** while pointing at new storage roots and a new dataset inventory.

This repo helps you:
- keep a single inventory (`targets_agri_circular.yaml`) of candidate sources,
...
See `AGRI_CIRCULAR_ADAPTATION.md` at the repo root for the migration plan from the chemistry package and the rationale for the new targets file.
...
```
+------------------+     +---------------------+     +-------------------------+
|  targets_agri_circular.yaml    |---->| pipeline_driver.py  |---->|  Queue JSONLs           |
...
- GREEN focus: EPA ECHO compliance data (FRS/RCRA/NPDES/DMR/SDWA), crosswalks, and AGROVOC ontology dumps.
- YELLOW candidates kept disabled until screened: Open Food Facts (ODbL), CommonPile subsets, GBIF/FAOSTAT record-level feeds.
- New worker stubs recommended (see AGRI_CIRCULAR_ADAPTATION.md): `echo_worker.py`, `agrovoc_worker.py`, and optional `geo_worker.py` for spatial tiles.
...
```bash
./run_pipeline.sh --targets targets_agri_circular.yaml
```

### Review pending YELLOW items
```bash
./run_pipeline.sh --targets targets_agri_circular.yaml --stage review
# or:
python3 review_queue.py --queue /data/agri_circular/_queues/yellow_pipeline.jsonl list
```
...
python3 review_queue.py approve \
  ...
  --reason "ECHO FRS bulk download; evidence ok" \
  --evidence-links "https://echo.epa.gov/tools/data-downloads#download" \
  --constraints "Attribution required per article license"

# Reject
python3 review_queue.py reject \
  --target openfoodfacts_products \
  --reviewer "Your Name" \
...
```bash
./run_pipeline.sh --targets targets_agri_circular.yaml --execute
```

### Build catalog
```bash
python3 catalog_builder.py --targets targets_agri_circular.yaml --output /data/agri_circular/_catalogs/global_catalog.json
```
...
### Configuration
- `targets_agri_circular.yaml` - dataset inventory + download/transform settings (schema v0.8)
...
python pipeline_driver.py --targets targets_agri_circular.yaml \
