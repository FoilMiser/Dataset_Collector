...
#
# Single wrapper script for the Agriculture + Circular Bioeconomy Pipeline.
# Orchestrates all stages with sensible defaults.
#
# Usage:
#   ./run_pipeline.sh --targets targets_agri_circular.yaml                    # Dry-run
#   ./run_pipeline.sh --targets targets_agri_circular.yaml --execute          # Full run
#   ./run_pipeline.sh --targets targets_agri_circular.yaml --stage download   # Specific stage
#
# Stages:
...
#   yellow    - Process YELLOW targets (domain-specific scrubbers)
#   pmc       - Download and chunk PMC articles (optional carry-over from chem pipeline)
...
Examples:
  ...
  $0 --targets targets_agri_circular.yaml

  # Execute with limits
  $0 --targets targets_agri_circular.yaml --execute --limit-targets 3 --limit-files 5

  # Run only classification stage
  $0 --targets targets_agri_circular.yaml --stage classify

  # Run PMC processing with caching (optional, legacy stage)
  $0 --targets targets_agri_circular.yaml --stage pmc --pmc-allowlist /path/to/allowlist.jsonl --execute --enable-cache
...
run_review() {
  ...
  local queues_root
  queues_root=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/agri_circular/_queues'))")
...
run_yellow() {
    ...
    if [[ -z "$YELLOW_ARGS" ]]; then
        echo "No yellow operations enabled. Use --pubchem-enable and/or --pmc-enable for legacy chem modules."
        return 0
    fi
...
run_pmc() {
    ...
    if [[ -z "$PMC_ALLOWLIST" ]]; then
        QUARANTINE_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals']['pools'].get('quarantine', '/data/agri_circular/pools/quarantine'))")
...
run_catalog() {
    ...
    CATALOGS_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('catalogs_root', '/data/agri_circular/_catalogs'))")
