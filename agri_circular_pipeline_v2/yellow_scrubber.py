...
This script does **not** perform agri-circular-specific transforms yet (future workers
can layer in routing-aware transforms). Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_agri_circular.yaml
    python yellow_scrubber.py --targets targets_agri_circular.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/agri_circular/_queues")).expanduser()
