...
def load_roots(targets_path: Optional[Path], overrides: argparse.Namespace) -> Roots:
    ...
    raw_root = Path(overrides.raw_root or g.get("raw_root", "/data/agri_circular/raw"))
    manifests_root = Path(overrides.manifests_root or g.get("manifests_root", "/data/agri_circular/_manifests"))
    logs_root = Path(overrides.logs_root or g.get("logs_root", "/data/agri_circular/_logs"))
...
def main() -> None:
    ...
    ap.add_argument("--targets-yaml", default=None, help="Path to targets_agri_circular.yaml for roots")
