    ...
    for url in urls:
        try:
            r = requests.get(url, timeout=timeout_s, 
                           headers={"User-Agent": f"agri-circular-scrubber/{VERSION}"})
            r.raise_for_status()
            return r.text, url
        except Exception:
            continue
...
def download_file(url: str, out_path: Path, timeout_s: int = 60) -> dict[str, Any]:
    ...
    with requests.get(url, stream=True, timeout=timeout_s,
                     headers={"User-Agent": f"agri-circular-scrubber/{VERSION}"}) as r:
        r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--pools-root", default="/data/agri_circular/pools")
    ...
    logs_dir = Path("/data/agri_circular/_logs").expanduser()
    ensure_dir(logs_dir)
