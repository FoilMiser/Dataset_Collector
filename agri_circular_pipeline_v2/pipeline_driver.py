...
Reads:
  - targets_agri_circular.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    agri_routing = (target.get("agri_routing", {}) or {})
    math_routing = (target.get("math_routing", {}) or {})

    subject = routing.get("subject") or agri_routing.get("subject") or math_routing.get("subject") or "agri_circular"
    domain = routing.get("domain") or agri_routing.get("domain") or math_routing.get("domain")
    category = routing.get("category") or agri_routing.get("category") or math_routing.get("category")
    level = coerce_int(routing.get("level"), coerce_int(agri_routing.get("level"), coerce_int(math_routing.get("level"))))
    granularity = routing.get("granularity") or agri_routing.get("granularity") or math_routing.get("granularity") or "target"
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"agri-circular-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_agri_circular.yaml (v0.8)")
    ...
    for t in targets:
        ...
        mr = t.get("agri_routing", {}) or t.get("math_routing", {}) or {}
        ...
        row = {
            ...
            "license_change_detected": license_change_detected,
            "output_pool": out_pool,
            "agri_domain": mr.get("domain"),
            "agri_category": mr.get("category"),
            "difficulty_level": mr.get("level"),
            "agri_granularity": mr.get("granularity"),
            # Generic routing (v2)
