...
run_review() {
  ...
  python -m collector_core.dc_cli review-queue --pipeline math -- --queue "$queue_file" --targets "$TARGETS" --limit 50 || true
}
...
run_catalog() {
  ...
  python -m collector_core.dc_cli catalog-builder --pipeline math -- --targets "$TARGETS" --output "${CATALOGS_ROOT}/catalog.json"
}
