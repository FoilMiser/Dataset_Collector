...
def load_roots(targets_path: Optional[Path], overrides: argparse.Namespace) -> Roots:
    ...
    raw_root = Path(overrides.raw_root or g.get("raw_root", "/data/math/raw"))
    manifests_root = Path(overrides.manifests_root or g.get("manifests_root", "/data/math/_manifests"))
    logs_root = Path(overrides.logs_root or g.get("logs_root", "/data/math/_logs"))
...
def main() -> None:
    ...
    ap.add_argument("--targets-yaml", default=None, help="Path to targets_math.yaml for roots")
