...
This script does **not** perform math-specific transforms yet (future workers
handle HF math filters, formal chunking, and PDF math extraction). Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_math.yaml
    python yellow_scrubber.py --targets targets_math.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_math.yaml")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/math/_queues")).expanduser()
