...
The recommended roots live in `targets_math.yaml -> globals`:
...
./run_pipeline.sh --targets targets_math.yaml --stage catalog
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `targets_math.yaml -> globals.screening` and per-target `yellow_screen` overrides.
