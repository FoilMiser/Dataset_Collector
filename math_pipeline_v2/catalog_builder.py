...
Builds a lightweight catalog for the v2 math pipeline layout. Summaries are
...
def build_catalog(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_root = Path(g.get("raw_root", "/data/math/raw"))
    screened_root = Path(g.get("screened_yellow_root", "/data/math/screened_yellow"))
    combined_root = Path(g.get("combined_root", "/data/math/combined"))
    final_root = Path(g.get("final_root", "/data/math/final"))
    ledger_root = Path(g.get("ledger_root", "/data/math/_ledger"))
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="targets_math.yaml")
