...
The recommended roots live in `../pipelines/targets/targets_physics.yaml -> globals`:
...
dc pipeline physics -- --targets ../pipelines/targets/targets_physics.yaml --stage classify --no-fetch

# Acquire GREEN and YELLOW (execute downloads)
dc run --pipeline physics --stage acquire --allow-data-root -- --queue /data/physics/_queues/green_pipeline.jsonl --bucket green --targets-yaml ../pipelines/targets/targets_physics.yaml --execute
dc run --pipeline physics --stage acquire --allow-data-root -- --queue /data/physics/_queues/yellow_pipeline.jsonl --bucket yellow --targets-yaml ../pipelines/targets/targets_physics.yaml --execute

# Screen, merge, catalog
dc run --pipeline physics --stage yellow_screen --allow-data-root -- --queue /data/physics/_queues/yellow_pipeline.jsonl --targets ../pipelines/targets/targets_physics.yaml --execute
dc run --pipeline physics --stage merge --allow-data-root -- --targets ../pipelines/targets/targets_physics.yaml --execute
dc catalog-builder --pipeline physics --allow-data-root -- --targets ../pipelines/targets/targets_physics.yaml --output /data/physics/_catalogs/catalog.json
```

### Notes

- YELLOW screening enforces "anything unclear is pitched"; see `../pipelines/targets/targets_physics.yaml -> globals.screening` and per-target `yellow_screen` overrides.
