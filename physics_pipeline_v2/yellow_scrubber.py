...
This script does **not** perform physics-specific transforms yet (future workers
handle domain extraction and PDF parsing). Instead it:
...
Usage examples:
    python yellow_scrubber.py --targets targets_physics.yaml
    python yellow_scrubber.py --targets targets_physics.yaml --limit 20 --output /tmp/yellow_plan.json
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Physics pipeline YELLOW review helper")
    ap.add_argument("--targets", required=True, help="Path to targets_physics.yaml")
    ...
    queues_root = Path(cfg.get("globals", {}).get("queues_root", "/data/physics/_queues")).expanduser()
    ...
    print("Physics YELLOW queue summary")
