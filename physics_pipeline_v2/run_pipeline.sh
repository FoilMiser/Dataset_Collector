...
#
# Wrapper script for the physics pipeline using the unified dc CLI.
#
set -euo pipefail
...
prefix = spec.prefix if spec else "physics"
...
run_classify() {
  ...
  python -m collector_core.dc_cli pipeline physics -- --targets "$TARGETS" $no_fetch
}

run_review() {
  local queue_file="$QUEUES_ROOT/yellow_pipeline.jsonl"
  echo -e "${BLUE}== Stage: review ==${NC}"
  python -m collector_core.generic_workers --domain physics review-queue -- --queue "$queue_file" --targets "$TARGETS" --limit 50 || true
}
...
run_acquire() {
  ...
  python -m collector_core.dc_cli run --pipeline physics --stage acquire -- \
    --queue "$queue_file" \
...
run_screen_yellow() {
  ...
  python -m collector_core.dc_cli run --pipeline physics --stage yellow_screen -- \
    --targets "$TARGETS" \
    --queue "$queue_file" \
    $EXECUTE
}
...
run_merge() {
  ...
  python -m collector_core.dc_cli run --pipeline physics --stage merge -- --targets "$TARGETS" $EXECUTE
}

run_catalog() {
  echo -e "${BLUE}== Stage: catalog ==${NC}"
  python -m collector_core.generic_workers --domain physics catalog -- --targets "$TARGETS" --output "${CATALOGS_ROOT}/catalog.json"
}
