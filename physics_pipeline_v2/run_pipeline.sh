...
#
# Wrapper script for the Physics Corpus Pipeline v2.
#
# Usage examples:
...
print(cfg.get("globals", {}).get("queues_root", "/data/physics/_queues"))
PY
)
CATALOGS_ROOT=$(python - << PY
import yaml
cfg = yaml.safe_load(open("${TARGETS}")) or {}
print(cfg.get("globals", {}).get("catalogs_root", "/data/physics/_catalogs"))
PY
)
