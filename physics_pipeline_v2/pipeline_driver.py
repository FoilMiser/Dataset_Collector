...
Reads:
  - targets_physics.yaml (schema v0.8)
  - license_map.yaml
  - denylist.yaml (v0.2)
...
def resolve_routing_fields(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    physics_routing = (target.get("physics_routing", {}) or {})

    subject = (
        routing.get("subject")
        or physics_routing.get("subject")
        or math_routing.get("subject")
        or "physics"
    )
    domain = routing.get("domain") or physics_routing.get("domain") or math_routing.get("domain")
    category = (
        routing.get("category") or physics_routing.get("category") or math_routing.get("category")
    )
    level = coerce_int(
        routing.get("level"),
        coerce_int(physics_routing.get("level"), coerce_int(math_routing.get("level"))),
    )
    granularity = (
        routing.get("granularity")
        or physics_routing.get("granularity")
        or math_routing.get("granularity")
        or "target"
    )
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                url,
                timeout=timeout_s,
                headers={"User-Agent": f"physics-corpus-pipeline/{VERSION}", **(headers or {})},
            )
            r.raise_for_status()
...
def main() -> None:
    ...
    ap.add_argument("--targets", required=True, help="Path to targets_physics.yaml (v0.8)")
    ...
    for t in targets:
        ...
        evaluation = {
            ...
            "physics_routing": {
                ...
                "level": pr.get("level"),
                "granularity": pr.get("granularity"),
            },
            "routing": routing,
        }
