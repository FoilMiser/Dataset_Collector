...
def resolve_roots(cfg: dict[str, Any]) -> Roots:
    ...
    return Roots(
        raw_root=Path(g.get("raw_root", "/data/physics/raw")),
        screened_root=Path(g.get("screened_yellow_root", "/data/physics/screened_yellow")),
        manifests_root=Path(g.get("manifests_root", "/data/physics/_manifests")),
        ledger_root=Path(g.get("ledger_root", "/data/physics/_ledger")),
        pitches_root=Path(g.get("pitches_root", "/data/physics/_pitches")),
    )
...
def canonical_record(raw: dict[str, Any], text: str, target_id: str, license_profile: str, license_spdx: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "routing": raw.get("routing") or raw.get("physics_routing") or raw.get("math_routing") or raw.get("route") or {},
