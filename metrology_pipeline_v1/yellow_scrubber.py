...
Legacy helper for chemistry/PMC transforms. Metrology/technical-report flows
typically skip this script; it's kept for compatibility with inherited targets.
...
def main() -> None:
    ap = argparse.ArgumentParser(description=f"Yellow Scrubber v{VERSION} (legacy chemistry/PMC helper)")
    ap.add_argument("--targets", required=True, help="targets.yaml v0.6+ (metrology package keeps compatibility)")
    ap.add_argument("--license-map", default=None)
    ap.add_argument("--field-schemas", default=None, help="field_schemas.yaml")
    ap.add_argument("--pools-root", default="/data/metrology/pools")
