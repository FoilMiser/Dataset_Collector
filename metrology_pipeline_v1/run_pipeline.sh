...
#
# Single wrapper script for the Metrology/Technical-Report Corpus Pipeline.
...
#   yellow    - Legacy chemistry/PMC helpers (typically skip for metrology)
#   pmc       - Legacy PMC OA helper (typically skip for metrology)
...
Options:
  ...
  --pubchem-enable        Enable PubChem extraction in yellow stage (legacy chemistry helper)
  --pmc-enable            Enable PMC allowlist generation in yellow stage (legacy chemistry helper)
...
run_download() {
    ...
    QUEUES_ROOT=$(python -c "import yaml; print(yaml.safe_load(open('$TARGETS'))['globals'].get('queues_root', '/data/metrology/_queues'))")
